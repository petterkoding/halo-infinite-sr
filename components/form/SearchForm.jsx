...
const SearchForm = () => {
    ...
    const styles ={
        ...
        input: "bg-slate-200/[.1] h-full w-[80%] md:w-96 p-3 border text-white"
    }
    ...
    return (
        <form className="mt-7 h-auto w-full flex flex-col justify-center" onSubmit={handleSubmit(onSubmit)}>
            <fieldset className="">
                <label htmlFor="searchForPlayer" className="text-white text-xs block">Search for Player</label>
                    <input
                        ...
                        />
                    <button className="px-4 h-full border w-auto border-slate-50/20 bg-slate-200/30 text-white uppercase hover:bg-cyan-600/90 transition duration-200 ease-out">
                        go
                    </button>
